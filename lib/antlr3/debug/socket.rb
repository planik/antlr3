...
module ANTLR3
  module Debug
    # rdoc ANTLR3::Debug::EventSocketProxy
    #
      ...
      def handshake
        unless @socket
          begin
            ...
            log!('  %s: %s', e.class, e.message)
            log!('  Backtrace: ')
            log!('  - %s', e.backtrace.join("\n  - "))
    ...
    # rdoc ANTLR3::Debug::RemoteEventSocketListener
    #
    ...
    class RemoteEventSocketListener < ::Thread
      ...
      def unpack_event(event)
        ...
        elements.map! do |elem|
          ...
          when /^"(.*)"$/ then parse_string(::Regexp.last_match(1))
          end
        end
