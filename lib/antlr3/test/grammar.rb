...
module ANTLR3
  module Test
    ...
    class Grammar
      ...
      def target_files(include_imports = true)
        ...
        for target_type in %w[lexer parser tree_parser]
          target_name = send(:"#{target_type}_file_name") and
      ...
      ##################################################################
      def compile(options = {})
        return unless options[:force] or stale?
      ...
      def build_dependencies
        ...
        return unless @source =~ /tokenVocab\s*=\s*(\S+)\s*;/

        foreign_grammar_name = ::Regexp.last_match(1)
    ...
    class Grammar::InlineGrammar < Grammar
      ...
      def inspect
        format('inline grammar %s (%s:%s)', name, @host_file, @host_line)
      end
      ...
      def write_to_disk
        ...
        return if test('f', @path) and MD5.digest(@source) == MD5.digest(File.read(@path))
    ...
    class Grammar::CompilationFailure < StandardError
      ...
      def initialize(grammar, command, status, output)
        ...
        message = format(<<-END.here_indent!, command, status, grammar, @output)
    ...
    class Grammar::FormatError < StandardError
      ...
      def initialize(source, file = nil)
        ...
        if file.nil? # inline
          ...
          message << format('bad grammar source in file %p', @file)
          message << ('-' * 80) << "\n"
          message << @source
          message[-1] == "\n" or message << "\n"
          message << ('-' * 80) << "\n"
