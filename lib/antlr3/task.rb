...
module ANTLR3
  ...
  class CompileTask < Rake::TaskLib
    ...
    def initialize(*grammar_files)
      ...
      options = grammar_files.last.is_a?(Hash) ? grammar_files.pop : {}
      @grammar_sets = []
    ...
    def grammar_set(*grammar_files)
      ...
      options = @options.merge(
        grammar_files.last.is_a?(Hash) ? grammar_files.pop : {}
      )
    ...
    class GrammarSet
      ...
      def clean
        ...
        return unless test('d', output_directory) and (Dir.entries(output_directory) - %w[. ..]).empty?
    ...
    | -- ignoring dependency
      ...
      def build_command(grammar)
        ...
        parts << grammar.path
        parts.map! { |t| escape(t) }.join(' ')
      end

      def escape(token)
        token = token.to_s.dup
        token.empty? and return(%(''))
        token.gsub!(%r{([^A-Za-z0-9_\-.,:/@\n])}n, '\\\\\\1')
        token.gsub!(/\n/, "'\n'")
        token
      end
    end
    ...
    class GrammarFile
      ...
      for attr in %i[output_directory load_path antlr_jar]
      ...
      def target_files(all = true)
        ...
        for target_type in %w[lexer parser tree_parser]
          for file in send(:"#{target_type}_files")
            targets << file
          end
        end
      ...
      def study
        ...
        @type = ::Regexp.last_match(1) || 'combined'
        if @source =~ /^\s*options\s*\{(.*?)\}/m
          option_block = ::Regexp.last_match(1)
          if option_block =~ /\s*language\s*=\s*(\S+)\s*;/
            @language = ::Regexp.last_match(1)
            LANGUAGES.has_key?(@language) or
              raise(Grammar::FormatError, format('Unknown ANTLR target language: %p', @language))
          end
          option_block =~ /\s*tokenVocab\s*=\s*(\S+)\s*;/ and
            @token_vocab = ::Regexp.last_match(1)
        end

        @source.scan(/^\s*import\s+(\w+\s*(?:,\s*\w+\s*)*);/) do
          list = ::Regexp.last_match(1).strip
    ...
    class GrammarFile::Imported < GrammarFile
      ...
      for attr in %i[load_path output_directory antlr_jar verbose group]
        class_eval(<<-END, __FILE__, __LINE__ + 1)
      def #{attr}
        @owner.#{attr}
      end
    ...
    class GrammarFile::FormatError < StandardError
      ...
      def initialize(source, file = nil)
        ...
        if file.nil? # inline
          ...
          message << format('bad grammar source in file %p\n', @file)
          message << ('-' * 80) << "\n"
          message << @source
          message[-1] == "\n" or message << "\n"
          message << ('-' * 80) << "\n"
