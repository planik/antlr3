  ...
  module Template
    module Builder
      ...
      def self.included(klass)
        super
        klass.is_a?(Class) and klass.extend(ClassMethods)
      end
    ...
    module RewriteBuilder
      ...
      def self.included(klass)
        super
        klass.is_a?(Class) and klass.extend(Builder::ClassMethods)
      end
    ...
    class Group < Module
      ...
      def define_template(name, source, parameters = nil)
        name = name.to_s.dup.freeze
        Context.define(self, name, parameters) do |tclass|
          self::TEMPLATES[name] = tclass
          ERB.new(source, trim_mode: '%').def_method(tclass, 'to_s')
      ...
      def alias_template(new_name, old_name)
        new_name = new_name.to_s.dup.freeze
        old_name = old_name.to_s
        context = self::TEMPLATES.fetch(old_name.to_s) do
          raise(NameError,
                format("undefined template `%s' for template group %p", old_name, self))
        end
    ...
    class Context
      ...
      def method_missing(method, *args)
        ...
        when SETTER_FORM then return(self[::Regexp.last_match(1)] = args.first)
        when ATTR_FORM
