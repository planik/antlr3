...
class TestRewritingLexerOutputDirectly < ANTLR3::Test::Functional
  ...
  example 'overlapping replace 2' do
    ...
    -> { stream.render }
      .should(raise_error do |error|
                error.to_s.should == 'operation (replace @ 1..2 : "foo") overlaps with previous operation (replace @ 0..3 : "bar")'
              end)
  end
  ...
  example 'replace range then insert at right edge' do
    ...
    -> { stream.render }
      .should(raise_error do |error|
                error.to_s.should == 'operation (insert-before @ 4 : "y") overlaps with previous operation (replace @ 2..4 : "x")'
              end)
  end
  ...
  example 'replace then replace superset' do
    ...
    -> { stream.render }
      .should(raise_error do |error|
                error.to_s.should == 'operation (replace @ 3..5 : "foo") overlaps with previous operation (replace @ 2..4 : "xyz")'
              end)
  end
  ...
  example 'replace then replace lower indexed superset' do
    ...
    -> { stream.render }
      .should(raise_error do |error|
                error.to_s.should == 'operation (replace @ 1..3 : "foo") overlaps with previous operation (replace @ 2..4 : "xyz")'
              end)
  end
end
...
class TestRewritingWithTokenStream2 < ANTLR3::Test::Functional
  ...
  example 'rendering over a range' do
    ...
    stream.original_string.should
    stream.render.should
    stream.render(0, 9).should
  ...
  example 'more rendering over a range' do
    ...
    stream.replace 4, 8, '0'
    stream.render.should
    stream.render(0, 17).should
    stream.render(4, 8).should
    stream.render(0, 8).should
    stream.render(12, 16).should
    stream.insert_after(17, '// comment')
    stream.render(12, 17).should
