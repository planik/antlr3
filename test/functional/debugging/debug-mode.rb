    ...
    super do
      timeout(2) do
        ...
        if $VERBOSE
          ...
          warn('sleeping for 0.1 seconds before retrying')
        end
        sleep(0.01)
        retry
      end

      @socket.readline.strip.should
      @socket.readline.strip.start_with?('grammar "').should
      ack
      loop do
...
class TestDebugGrammars < ANTLR3::Test::Functional
  ...
  def parse(grammar, rule, input, options = {})
    ...
    listener = options[:listener] or debugger = ANTLRDebugger.new(port = 49_100)
