...
class TestTreeWizard < Test::Unit::TestCase
  ...
  def test_no_repeats_visit
    ...
    elements = []
    @wizard.visit(tree, B) do |node, _parent, _child_index, _labels|
      elements << node.to_s
    end
  ...
  def test_no_repeats_visit2
    ...
    elements = []
    @wizard.visit(tree, C) do |node, _parent, _child_index, _labels|
      elements << node.to_s
    end
  ...
  def test_repeats_visit2
    ...
    elements = []
    @wizard.visit(tree, A) do |node, _parent, _child_index, _labels|
      elements << node.to_s
    end
  ...
  def context(node, parent, index)
    format('%s@%s[%d]', node.to_s, (parent || 'nil').to_s, index)
  end
  ...
  def test_repeats_visit_with_context
    ...
    elements = []
    @wizard.visit(tree, B) do |node, parent, child_index, _labels|
  ...
  def test_repeats_visit_with_null_parent_and_context
    ...
    elements = []
    @wizard.visit(tree, A) do |node, parent, child_index, _labels|
      elements << context(node, parent, child_index)
    end
  ...
  def test_visit_pattern
    ...
    elements = []
    @wizard.visit(tree, '(A B)') do |node, _parent, _child_index, _labels|
      elements << node.to_s
    end
  ...
  def test_visit_pattern_multiple
    ...
    elements = []
    @wizard.visit(tree, '(A B)') do |node, parent, child_index, _labels|
      elements << context(node, parent, child_index)
    end
  ...
  def labeled_context(node, parent, index, labels, *names)
    ...
    format('%s@%s[%d]%s', node.to_s, (parent || 'nil').to_s, index, suffix)
  end
