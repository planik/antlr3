...
module ANTLR3
  module Test
    ...
    class Session
      ...
      def tests
        each_type.inject([]) do |tests, (_type, klass)|
      ...
      def run_tests(options = {})
        ...
        Progress.run(total_weight, output: @output) do |bar|
          each_type { |_type, klass| @success &= klass.run(bar, options) }

          bar.clear_line
          report_failures(bar)
          Pager.open { |pager| report_failures(pager) } unless @success or !($stdout.tty? && $stdin.tty?)
        end
        @success
      end
      ...
      def save
        ...
        each_type { |_type, klass| klass.save(@directory) }
      end
      ...
      def summarize(out)
        out.center do
          each_type { |_type, klass| klass.summarize(out, @options) }
        end
      end

      def open(file, options = {}, &block)
        test('d', @directory) or Dir.mkdir(@directory)
        OutputDevice.open(@directory / file, options, &block)
      end
    ...
    class Session
      class Item
        class << self
          ...
          def tests = instances.values
          def failures = tests.select { |t| t.ran? and t.failed? }

          def report_failures(out)
            failures = tests.select { |t| t.ran? and t.failed? }
            return if failures.empty?

            title and out.foreground(:cyan) do
              out.puts("From #{title}:")
            end
          ...
          def summarize(out, _options)
            return unless out and title
        ...
        def weight = @weight ||= compute_weight
        def failed? = false
        def report_failure(out) end # do nothing

        private

        def compute_weight = 1
      ...
      class TestFile < Item
        ...
        def run(progress_bar, options = {})
          if progress_bar.is_a?(Hash) then options = progress_bar
                                           progress_bar = nil end
          environment('COLUMNS' => progress_bar.width) if progress_bar

          @results = ruby(@path, options) do |_cmd, out, _err|
            progress(progress_bar, out)
          end

          if @results.stdout =~ DATA_RX
            report = ::Regexp.last_match.pre_match
        ...
        def source = @source ||= File.read(@path)
        def failed? = @results and @results.failed?
      ...
      Time: Time elapsed while running all examples (in seconds)
        ...
        def self.table_head = %w[Name Status #T :) :| :( E! IM SX AC Time]
        def self.title = 'Functional Test Results'

        def type = 'functional'
        def category = File.basename(File.dirname(path))
        def description = "[#{type}] #{category}: #{name}"
        ...
        def summarize
          report =
            ...
            when 1 then @summary.report(description: 'test error'.cyan)
              ...
              @summary.report(
                description: format('huh? (%p)', @status).yellow
              )
            end
          [name, *report]
        end
        ...
        def compute_weight
          ...
          for name, body in grammars
            body.scan(/^ *import +(.+?);/) do
              ...
              imported |= ::Regexp.last_match(1).split(/\s*,\s*/)
            end
          end
          compile_weight = (compile_weight - imported.length).at_least(0)
          @source = nil

          (super + GRAMMAR_WEIGHT * compile_weight)
        rescue StandardError
          10
        end
      end
      ...
      class UnitTest < TestFile
        ...
        def self.title = 'Unit Test Results'
        def self.table_head = %w[Name Status #T :) :| :( E! Time]

        def type = 'unit'
        def name = super.gsub(/test-/, '')
      ...
      class PerformanceTest < Item
        ...
        def by_products = [@output_directory]

        def run(progress_bar, _options = {})
          if progress_bar.is_a?(Hash) then options = progress_bar
                                           progress_bar = nil end
          ...
          for sample in @samples
            ...
            ruby(sample.script) do |_cmd, out, _err|
              progress(progress_bar, out)
          ...
          (@samples.none? { |s| s.error })
        end

        def failed? = @samples.any? { |sample| sample.error }

        def report_failure(out)
          out.foreground(:yellow) { out.puts(@path) }
          for sample in @samples
            next unless err = sample.error
      ...
      class ProfileTest < PerformanceTest
        def self.title = nil
        def self.directory_name = 'profiling-reports'
        def script_name(name) = "profile_#{name}"
        def type = 'profile'
        def summarize(*) end
      ...
      Real: >-
        ...
        def self.title = 'Benchmarking Performance Data'
        def self.directory_name = 'benchmark-data'

        def script_name(name) = "bench_#{name}"
        def type = 'benchmark'
        ...
        def summarize(table)
          ...
          make_row = proc do |action, item, stat|
            ...
            r = %i[total utime stime real].map! do |t|
              '%.3f' % stat.send(t).mean
            end
          ...
          for sample in @samples
            next unless data = sample.performance_data
