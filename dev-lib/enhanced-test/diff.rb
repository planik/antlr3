  ...
  module Test
    module Diff
      ...
      def wrapped_inspect(list)
        ...
        for item in list
          ...
          if item.length > leftover
            ...
            if line = add_line[line << item[0, i]]
              item = item[i..-1]
            end
          end
          line << item
      ...
      def compute_lcs(list_a, list_b)
        m = list_a.length
        n = list_b.length
        lcs_matrix = Array.new(m + 1) do |_row|
          Array.new(n + 1, 0)
        end
        m.times do |i|
          n.times do |j|
            lcs_matrix[i + 1][j + 1] = if list_a[i] == list_b[j] then lcs_matrix[i][j] + 1
                                       else
                                         max(lcs_matrix[i + 1][j], lcs_matrix[i][j + 1])
                                       end
          end
        end
        lcs_matrix
      ...
      def partial_diff(out, a, b, lcs, i = a.length, j = b.length)
        ...
        elsif j > 0 and i.zero? || lcs[i][j - 1] >= lcs[i - 1][j]
