...
module ANTLRDoc
  class Table
    ...
    def self.def_template(signature, source)
      t = ERB.new(source.chomp.fixed_indent(0), trim_mode: '%')
    ...
    def initialize(data = {})
      ...
      @columns = data.fetch(:columns, []).map do |col|
        col.is_a?(Hash) ? col.symbolize_keys : { title: col }
      end
      @body = data.fetch(:body, []).map do |row|
        row = row.map do |cell|
          cell.is_a?(Hash) ? cell.symbolize_keys : { content: cell }
        end
        { cells: row }
      end

      return unless @stripes = data.fetch(:stripes) { %w[odd even] }
