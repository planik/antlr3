...
module ANTLRDoc
  class PropertyGroup < ::Hash
    ...
    def load(property, path)
      ...
      value =
        ...
        when /\.erb$/i  then ERB.new(data, trim_mode: '%')
        else data
        end
  ...
  class Article < PropertyGroup
    ...
    def empty?
      !(test('f', source_file) && File.size?(source_file))
    end
    ...
    def generate
      ...
      eval_template(guide.template, body:, outline:)
    end
      ...
      for line in source.split($/)
        ...
        if (line =~ /^« *(\S+) *(.*)/) ... (line =~ /^»/)
          if tag.nil?
            ...
            options = $project.yaml.load("{ #{::Regexp.last_match(2)} }")
    ...
    def content_link(_full, name, text)
